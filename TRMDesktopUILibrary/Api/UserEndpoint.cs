...
namespace TRMDesktopUILibrary.Api
{
    public class UserEndpoint : IUserEndpoint
    {
        ...
        public async Task AddUserToRole(string userId, string roleName)
        {
            ...
            using (HttpResponseMessage response = await _apiClient.ApiClient.PostAsJsonAsync("/api/User/Admin/AddRole", data))
            {
                if (response.IsSuccessStatusCode == false)
                {
                    throw new Exception(response.ReasonPhrase);
        ...
        {
            var data = new { userId, roleName };
            using (HttpResponseMessage response = await _apiClient.ApiClient.PostAsJsonAsync("/api/User/Admin/RemoveRole", data))
            {
                if (response.IsSuccessStatusCode == false)
                {
                    throw new Exception(response.ReasonPhrase);
